...
export default function SetupScreen() {
  ...
  const notify = useCallback(
    (message: string) => {
      if (currentFeedback.visualFeedback) setToastMessage(message);
    },
    [currentFeedback.visualFeedback],
  );

  const onSelectFontSize = useCallback(
    (fontSize: SetupType["fontSize"]) => {
      dispatch(actions.updateFontSize(fontSize));
      notify("Tamanho da letra atualizado!");
    },
    [dispatch, notify],
  );
  ...
  const onSelectContrastLevel = useCallback(
    (contrastLevel: SetupType["contrastLevel"]) => {
      ...
      notify("Nível de contraste atualizado!");
    },
    [dispatch, notify],
  );
  ...
  const onSelectFeedback = useCallback(
    (feedback: FeedbackProps) => {
      ...
      notify(`${feedback.title} ${feedback.enabled ? "desativado" : "ativado"}!`);
    },
    [dispatch, notify],
  );
